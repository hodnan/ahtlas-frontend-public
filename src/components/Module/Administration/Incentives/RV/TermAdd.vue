<script setup>
  ...
  const freeToChange = ref(true)
  const TermCopyId = ref()
  ...
  const copy = async (t) => {
    try {
      freeToChange.value = false
      if (t != 0) {
        await module
          .getShow(`api/administration/incentives/rv/terms-copy/show/${t}`, ['term_copy'])
          .then(() => {
            let temp = JSON.parse(JSON.stringify(module.data?.term_copy ?? []))
  ...
  watch(
    ...
    (newFilter) => {
      ...
      // console.log(!TermCopyId.valeu, TermCopyId.value)
      if (freeToChange.value) {
        form.value.basket = [
          {
            indicator_id: null,
            range: null,
        ...
        form.value.deflator = []
        form.value.elimination = [
          {
            indicator_id: 747,
            operation: '<=',
            target: '1',
            value: null,
            calc: { id: 1, label: 'Total' },
            calcs: [{ id: 1, label: 'Total' }]
          }
        ]

        form.value.campaign = module.data.sectors
          .find((item) => item.id == newFilter)
          ?.name.toUpperCase()
          .normalize('NFD')
          .replace(/[\u0300-\u036f]/g, '')
          .replace(/[^A-Z_ ^0-9]/g, '') // Mantém letras do alfabeto, o caractere "_" e espaços
  ...
  const submit = async () => {
    try {
      await module.setStore('api/administration/incentives/rv/terms/store', form.value)
      if (notify.success) {
        closeDialog()
  ...
  <v-form @submit.prevent="submit">
    ...
    <v-card-actions class="px-4 pt-1 pb-0">
      <v-autocomplete
        ...
        item-value="id"
      />
      <v-btn :loading="module.showing" @click.stop="copy(TermCopyId ?? 0)">copiar</v-btn>
      <v-spacer></v-spacer>
