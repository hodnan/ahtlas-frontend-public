<script setup>
  import { ref } from 'vue'
  import { moduleStore, notifyStore, authStore } from '@/store'
  ...
  const submit = async () => {
    try {
      await module.setStore(`api/administration/intelligence/kpi-sources/store`, form.value)
      if (notify.success) {
        closeDialog()
        await module.getDataPart('api/administration/intelligence/kpi-sources', ['kpiSource'])
      }
    } catch (error) {
      console.log(error)
    }
