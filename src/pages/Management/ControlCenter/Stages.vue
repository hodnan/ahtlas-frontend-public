<script setup>
  ...
  import StageUpdate from '@/components/Module/Management/ControlCenter/StageUpdate.vue'
  ...
  const filter = ref({
    month_ref: urlParams.get('month_ref') ?? null,
    date_ref: urlParams.get('date_ref') ?? null,
    created_at: urlParams.get('created_at') ?? moment().format('YYYY-MM-DD')
  })
  ...
  const download = () => {
    ...
    module.getDownload('api/management/control-center/stage-download', urldata)
  }
  ...
  watch(
    ...
    debounce(async () => {
      ...
      await module.getDataPart(
        'api/management/control-center/stages',
        ['stages', 'results'],
        filter
      )
    }, 500),
...
<template>
  ...
  <v-card class="mt-2 table_card">
    <v-card-text class="d-flex py-1 px-2">
      <v-icon icon="mdi-counter" class="text-brand_txt pt-2" />
      <div class="pt-1 px-3 text-brand_txt" style="min-width: 120px">Stages</div>
      <v-spacer />
      <v-btn
        density="comfortable"
        color="brand_roxo2"
  ...
  >
    <StageUpdate v-model="dialogStage" :stage="stage" />
  </CoreDialog>
</template>
<style scoped>
  .hover-color:hover {
