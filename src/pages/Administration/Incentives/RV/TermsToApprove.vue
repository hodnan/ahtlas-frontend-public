...
import TermApprove from '@/components/Module/Administration/Incentives/RV/TermApprove.vue'
...
watch(
  ...
  async (newFilter) => {
    ...
    module.getDataPart('api/management/myteam/terms-to-approve', ['terms'], filter.value)
  },
  { deep: true }
)
const dialogOpem = (event, item) => {
  dialogShow.value = true
  termName.value = item.item.term_name
...
<template>
    ...
    <v-card-text class="py-0 pb-3">
      ...
      >
        <template #[`item.status.label`]="{ item }">
          <v-chip variant="flat" :color="item?.status?.color" size="small">
            {{ item?.status?.label }}
          </v-chip>
        </template>
      </v-data-table-virtual>
  ...
  >
    <TermApprove v-model="dialogShow" :id="termId" />
  </CoreDialog>
</template>
